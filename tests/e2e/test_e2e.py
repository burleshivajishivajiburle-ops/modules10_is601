...
def test_calculator_add(page, fastapi_server):
    ...
    # Wait until the fetch call has updated the DOM before making an assertion (async fetch may take a beat).
    page.wait_for_selector('#result:has-text("Result: 15")', timeout=3000)
...
def test_calculator_divide_by_zero(page, fastapi_server):
    ...
    # Wait for the async request to finish so the DOM reflects the error before asserting.
    page.wait_for_selector('#result:has-text("Error: Cannot divide by zero!")', timeout=3000)
